...
public class StartPage extends BasePage {
    ...
    public StartPage checkText() {
        ...
        Assert.assertEquals("Текст " + text + " отсутствует на странице", text, numberOfRemainingItems.getText().trim());
        logger.info("Проверка присутствия текста '5 of 5 remaining'");
        return this;
    }

    @Step("Проверить, что элемент  '{nameOfItem}' в списке не зачеркнут")
    public StartPage checkItem(String nameOfItem) {
        for (WebElement item : listOfCheckboxes) {
            ...
            if (checkbox.getText().trim().equals(nameOfItem)) {
                ...
                logger.info("Проверка того, что элемент '" + nameOfItem + "' в списке не зачеркнут");
                return this;
            }
        }
        Assert.fail("Элемента списка '" + nameOfItem + "' нет на cтранице");
        return this;
    }
